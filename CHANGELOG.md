...
- **CI now scores compressed output against dart-sass, not only expanded.** The
  ...
  **compressed** CSS dart-sass 1.104.1 emits for the same 11,766 cases,
  ...
  and it refuses to run if its dart-sass or sass-spec header disagrees with the
  pins, if its `cases:` header disagrees with its body, or if fewer cases were
  scored than the baseline records -- a case that stops being measured cannot be
  seen to fail.
