...
- **CI now scores compressed output against dart-sass, not only expanded.** The
  ...
  **compressed** CSS dart-sass 1.104.1 emits for the same 11,766 cases,
  ...
  and it refuses to run unless all four of the manifest's headers are present
  and agree with the pins: the output style, the dart-sass version, the
  sass-spec commit, and a case count matching the body. It also refuses a run
  that scored fewer cases than the baseline records -- a case that stops being
  measured cannot be seen to fail.
