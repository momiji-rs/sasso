...
| a built-in argument passed twice — `color.mix(red, blue, 10%, $color1: green)` ([#147](https://github.com/momiji-rs/sasso/issues/147)) | `Argument $color1 was passed both by position and by name.` | the positional wins and the call succeeds, in every family, so a stylesheet dart rejects compiles. dart raises this before `Missing argument`, before the positional-count error and before an unknown name, even with a rest parameter. A *user* callable is rejected, with the wrong message — §3. Measured 2026-09-19 |
